...
private:
    // 创建单调队列
    // 不需要保证队列中拥有所有元素
    // 只需要保证队列的头部是最大的  并且从尾端加入的元素是整个队列是由小到大的
        class MyQueue{ // 单调队列  元素由大到小
            public:
                deque<int> que; //  用双端队列去实现

                // pop()
                void pop(int value){ 
                    if(!que.empty() && que.front()==value){
                        que.pop_front();  // 从开头处去除元素
                    }
                }
...
public:
    vector<int> maxSlidingWindow(vector<int>& nums, int k) {
        ...
        result.push_back(myque.front());  // 记录最大值
        for(int i=k;i<nums.size();++i){
            myque.push(nums[i]); // 看加入的多大
            myque.pop(nums[i-k]); // 看去除的是不是最大的
